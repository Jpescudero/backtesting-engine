...
def test_equity_and_net_curves_are_finite() -> None:
    ...
    signals = np.array([1, 0, 0, -1], dtype=np.int8)
    result = run_backtest_with_signals(data, signals, config=config, cost_model=model)

    assert not np.isnan(result.equity).any()
    assert result.equity_net is not None
    assert result.equity_net.shape == result.equity.shape
    assert not np.isnan(result.equity_net).any()
